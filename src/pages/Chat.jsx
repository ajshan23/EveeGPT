  ...
  return (
      ...
      <div className="flex flex-col w-full h-full bg-[#343541]">
        ...
        <div className="w-screen h-screen px-20 ">
          <div className=" flex flex-col w-full h-full gap-5 mt-10">
            <div>
              <div className="bgstyle py-2 scroll-container w-[60%] h-custom  border-[#502464] border-2 rounded-lg border-opacity-30 overflow-x-hidden overflow-y-scroll scroll-smooth bg-cover bg-center"  >
   ...
   allResults.map((item, index) => (
    <div
      ...
      key={index}
    >
      <div className={`w-10 h-10 rounded-full flex justify-center items-center overflow-hidden ${item.role==='user'?'bg-[#E8E8E8] ml-4':' bg-white mr-4 ' }`}>
       {item.role==='user'? <img src={user}  className="rounded w-10 h-8" />: <img src={logo} className="w-10 h-10" />}
      </div>
      <div className={`relative mt-2 w-full h-full  flex ${item.role==='user' ?'bg-[#3f4f63] rounded-tr-0 rounded-tl-lg rounded-b-lg' :'bg-[#502464] rounded-tl-0 rounded-tr-lg rounded-b-lg'}`} >
        {item.role==='user'?<div className="absolute -rotate-90 -right-3 cuscss"><FaLocationPin color="#3f4f63" size={21}/></div>:<div className="absolute top-custom  rotate-90"><FaLocationPin color="#502464" size={21}/></div>}
        
        <span className="px-5 py-5 text-white">{item.parts}</span>
      </div>
    </div>
  ))
}
